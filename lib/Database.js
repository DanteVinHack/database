...
class Database {
  ...
  async _getAllFileInDir(fileNames = []) {
  ...
  async readDir() {
    ...
    const data = this._getAllFileInDir(files);
  ...
  async saveData(name, data = {}) {
    ...
    if (!searchData.length) {
      ...
      fs.appendFile(path.join(this.#getFilePath(name)), JSON.stringify(data), err => {
        ...
        else console.log('Successfull saved')
      })
    }
  }
...
const usersBase = new Database(path.join(__dirname, '..', 'database', 'users')) 
